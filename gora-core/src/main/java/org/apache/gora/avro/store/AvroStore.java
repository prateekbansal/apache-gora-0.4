...
public class AvroStore<K, T extends PersistentBase>
  ...
  protected Encoder createEncoder() throws IOException {
    switch(codecType) {
      case BINARY:
        return new EncoderFactory().binaryEncoder(getOrCreateOutputStream(), null); //  new BinaryEncoder(getOrCreateOutputStream());
      case JSON:
        return new EncoderFactory().jsonEncoder(schema, getOrCreateOutputStream()); // new JsonEncoder(schema, getOrCreateOutputStream());
    }
    return null;
  }
  ...
  protected Decoder createDecoder() throws IOException {
    switch(codecType) {
      case BINARY:
        return new DecoderFactory().binaryDecoder(getOrCreateInputStream(), null); // new  BinaryDecoder(getOrCreateInputStream());
      case JSON:
        return new DecoderFactory().jsonDecoder(schema, getOrCreateInputStream()); // new JsonDecoder(schema, getOrCreateInputStream());
    }
    return null;
  }

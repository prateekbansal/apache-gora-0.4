...
public class PersistentDatumReader<T extends PersistentBase>
  ...
  throws IOException {
    ...
    if(resolvingDecoder == null) {
      resolvingDecoder = new DecoderFactory().resolvingDecoder(rootSchema, rootSchema, decoder);
      //new FakeResolvingDecoder(rootSchema, decoder);
    ...
    if(expected.equals(rootSchema) && readDirtyBits) {
      ...
      for (Field f : expected.getFields()) {
        if(readableFields[f.pos()]) {
          ...
          getSpecificData().setField(record, name, pos, read(oldDatum, f.schema(), in));
        }
      }
